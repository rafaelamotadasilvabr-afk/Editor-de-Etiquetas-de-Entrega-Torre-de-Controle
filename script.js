...
function textForVolume(text){
  // A quantidade não é escrita no bloco livre.
  // A numeração é aplicada somente no campo original "Volume" da etiqueta.
  return text || '';
}


// Posição proporcional do valor localizado abaixo do título "Volume"
// no padrão atual das etiquetas Azul Logística.
// A posição é proporcional, portanto acompanha etiquetas coladas em resoluções diferentes.
const VOLUME_FIELD = {
  centerX: 0.170,
  centerY: 0.512,
  clearWidth: 0.145,
...
function drawVolumeField(targetCtx, targetCanvas, current, total){
  if(total <= 1) return;

  const w = targetCanvas.width;
  const h = targetCanvas.height;

  const centerX = Math.round(w * VOLUME_FIELD.centerX);
  const centerY = Math.round(h * VOLUME_FIELD.centerY);
  const clearW = Math.round(w * VOLUME_FIELD.clearWidth);
  const clearH = Math.round(h * VOLUME_FIELD.clearHeight);
  const left = Math.round(centerX - clearW / 2);
  const top = Math.round(centerY - clearH / 2);

  const value = `${current}/${total}`;

  targetCtx.save();

  // Apaga somente o valor original (ex.: 1/1), preservando o título "Volume" e as bordas.
  targetCtx.fillStyle = '#ffffff';
  targetCtx.fillRect(left, top, clearW, clearH);

  // Ajusta a fonte automaticamente para quantidades grandes, como 150/150.
  let fontSize = Math.max(9, Math.round(w * 0.028));
  const maxTextWidth = clearW * 0.92;
  do {
    targetCtx.font = `700 ${fontSize}px Arial, Helvetica, sans-serif`;
    if(targetCtx.measureText(value).width <= maxTextWidth || fontSize <= 8) break;
    fontSize -= 1;
  } while(fontSize > 8);
  ...
  targetCtx.font = `700 ${fontSize}px Arial, Helvetica, sans-serif`;
  targetCtx.fillText(value, centerX, centerY);
...
function drawLabel(){
  ...
  const displayText = textForVolume(text);
  if(text.trim()){
    if(isPhoneOnly(text)){
  ...
  // Na pré-visualização, mostra a primeira etiqueta do lote.
  drawVolumeField(ctx, canvas, 1, totalVolumes);
  setZoom(zoom);
}
...
function renderExportCanvas(scale = EXPORT_SCALE, volumeIndex = 1, totalVolumes = 1){
  ...
  const text = textForVolume(baseText);
  ...
  // Numera cada etiqueta somente dentro do campo original "Volume".
  drawVolumeField(exportCtx, exportCanvas, volumeIndex, totalVolumes);
...
function printCanvas(){
  ...
  if(totalVolumes > 20){
    const confirmed = window.confirm(`Serão geradas ${totalVolumes} etiquetas numeradas de 1/${totalVolumes} até ${totalVolumes}/${totalVolumes}. Deseja continuar?`);
    if(!confirmed) return;
  }
