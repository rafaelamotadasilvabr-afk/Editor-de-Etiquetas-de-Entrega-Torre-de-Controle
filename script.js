...
/*
  Campo original "Volume" da etiqueta.
  As medidas são proporcionais ao tamanho da imagem, portanto funcionam
  mesmo quando a etiqueta é colada com outra resolução.
*/
const VOLUME_FIELD = {
  centerX: 0.181,
  centerY: 0.496,
...
function drawVolumeField(targetCtx, targetCanvas, current, total){
  ...
  const centerX = Math.round(canvasWidth * VOLUME_FIELD.centerX);
  const centerY = Math.round(canvasHeight * VOLUME_FIELD.centerY);
  const clearWidth = Math.round(canvasWidth * VOLUME_FIELD.clearWidth);
  const clearHeight = Math.round(canvasHeight * VOLUME_FIELD.clearHeight);
  const left = Math.round(centerX - clearWidth / 2);
  const top = Math.round(centerY - clearHeight / 2);
  const value = `${current}/${total}`;

  targetCtx.save();

  // Cobre somente o valor antigo, mantendo o título Volume e as bordas.
  targetCtx.fillStyle = '#ffffff';
  targetCtx.fillRect(left, top, clearWidth, clearHeight);

  // Ajusta automaticamente para quantidades grandes, como 150/150.
  let fontSize = Math.max(9, Math.round(canvasWidth * 0.030));
  const maxTextWidth = clearWidth * 0.90;

  while(fontSize > 8){
    targetCtx.font = `700 ${fontSize}px Arial, Helvetica, sans-serif`;
    if(targetCtx.measureText(value).width <= maxTextWidth) break;
    fontSize -= 1;
  }
...
function drawLabel(){
  ...
  // A pré-visualização mostra a primeira etiqueta do lote.
...
function renderExportCanvas(scale = EXPORT_SCALE, volumeIndex = 1, totalVolumes = 1){
  ...
  // Numeração aplicada exclusivamente dentro do campo original Volume.
